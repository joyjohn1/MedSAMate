...
# --------------------------------------------------------------
from sam_med2d_funcs import *  # SAM-Med2D相关工具函数
...
def savestl(mask, spacing, subjname, savepath, smoothfactor=2.0):
    ...
    seg = ndimage.filters.gaussian_filter(seg, sigma=smoothfactor, truncate=8.0)  # 高斯平滑
    Seg = np.zeros(mask.shape)
    Seg[TL_x:BR_x, TL_y: BR_y, mask_id_up:mask_id_low] = seg  # 恢复到原始尺寸
    # 用marching cubes提取表面
...
class Ui_MainWindow(QObject):
    ...
    def _create_actions(self, QMainWindow):
        ...
        actions = [self.actionAdd_DICOM_Data, self.actionAdd_NIFTI_Data, self.actionAdd_IM0BIM_Data,
                   self.actionAdd_STL_Data, self.actionAdd_Load_Universal_model, self.actionAdd_Load_Lungseg_model]
    ...
    def _create_left_panel(self, QMainWindow):
        """创建左侧面板 - sam_ui(1).py样式：垂直分布1:2:1"""
        # 创建左侧面板容器，设置明确背景色
        ...
        self.verticalLayout_6.setStretch(1, 2)  # Basic Info
        self.verticalLayout_6.setStretch(2, 1)  # Contrast Adjustment
        self.verticalLayout_6.setStretch(3, 0)  # Annotation (hidden by default)

        self.horizontalLayout_5.addWidget(self.left_panel_widget)

    def _create_main_toolbar(self, QMainWindow):
        """主工具栏 - sam_ui(1).py样式：标题标签 + QFrame"""
    ...
    def _create_basic_info(self, QMainWindow):
        """基本信息区域 - sam_ui(1).py样式：标题标签 + QFrame"""
        ...
        # 信息文本框 - 跨两列
        self.textBrowser = QTextBrowser(self.frame_basic)
        self.textBrowser.setObjectName("textBrowser")
        self.textBrowser.setFont(self.normal_font)
        ...
        """)
        self.textBrowser.setMinimumHeight(80)
        self.gridLayout_basic.addWidget(self.textBrowser, 2, 0, 4, 2)

        self.verticalLayout_basic.addWidget(self.frame_basic)
        self.verticalLayout_basic.setStretch(0, 0)
        self.verticalLayout_basic.setStretch(1, 1)
        self.verticalLayout_6.addLayout(self.verticalLayout_basic)

    def _create_contrast_adjustment(self, QMainWindow):
        """对比度调整区域 - sam_ui(1).py样式：标题标签 + QFrame"""
        ...
        self.frame_contrast.setMinimumHeight(140)
        ...
        self.window_level_slider.setFixedHeight(24)
        ...
        self.window_width_slider.setFixedHeight(24)
    ...
    def _create_annotation_panel(self, QMainWindow):
        """标注面板 - sam_ui(1).py样式：标题标签 + QFrame"""
    ...
    def _create_viewer_area(self, QMainWindow):
        """创建右侧视图区域 - 修复右下角为VTK体绘制视图"""
        self.gridLayout_2 = QGridLayout()
        self.gridLayout_2.setSpacing(4)
        self.gridLayout_2.setContentsMargins(4, 4, 4, 4)

        # 初始化VTK相关变量 - 启动时不加载默认数据，保持空白
        self.pathDicomDir = ""
        self.reader = None  # 初始时不创建reader

        # XY视图 (横断面) - 初始为灰色背景
        self.horizontalLayout = QHBoxLayout()
        self.frame_XY = QFrame(self.centralwidget)
        self.frame_XY.setFrameShape(QFrame.NoFrame)
        self.frame_XY.setObjectName("frame_XY")
        self.frame_XY.setStyleSheet("background-color: #808080; border: 2px solid #666;")
        self.id_XY = "XY"
        ...
        ren_xy.SetBackground(0.5, 0.5, 0.5)  # 灰色背景 #808080
        self.vtkWidget.GetRenderWindow().AddRenderer(ren_xy)
        self.viewer_XY = None  # 初始不设置viewer

        self.horizontalLayout.addWidget(self.vtkWidget)

        self.verticalSlider_XY = QScrollBar(self.centralwidget)
        self.verticalSlider_XY.setObjectName(u"verticalSlider_XY")
        ...
        self.verticalSlider_XY.setEnabled(False)  # 初始禁用滑块
        self.horizontalLayout.addWidget(self.verticalSlider_XY)
        self.gridLayout_2.addLayout(self.horizontalLayout, 0, 0, 1, 1)

        # YZ视图 (矢状面) - 初始为灰色背景
        self.horizontalLayout_2 = QHBoxLayout()
        self.frame_YZ = QFrame(self.centralwidget)
        self.frame_YZ.setFrameShape(QFrame.NoFrame)
        self.frame_YZ.setObjectName("frame_YZ")
        self.frame_YZ.setStyleSheet("background-color: #808080; border: 2px solid #666;")
        self.id_YZ = "YZ"
        ...
        ren_yz.SetBackground(0.5, 0.5, 0.5)  # 灰色背景 #808080
        self.vtkWidget2.GetRenderWindow().AddRenderer(ren_yz)
        self.viewer_YZ = None  # 初始不设置viewer

        self.horizontalLayout_2.addWidget(self.vtkWidget2)

        self.verticalSlider_YZ = QScrollBar(self.centralwidget)
        self.verticalSlider_YZ.setObjectName(u"verticalSlider_YZ")
        self.verticalSlider_YZ.setStyleSheet(self.verticalSlider_XY.styleSheet())
        self.verticalSlider_YZ.setOrientation(Qt.Vertical)
        self.verticalSlider_YZ.setEnabled(False)  # 初始禁用滑块
        self.horizontalLayout_2.addWidget(self.verticalSlider_YZ)
        self.gridLayout_2.addLayout(self.horizontalLayout_2, 0, 1, 1, 1)

        # XZ视图 (冠状面) - 初始为灰色背景
        self.horizontalLayout_3 = QHBoxLayout()
        self.frame_XZ = QFrame(self.centralwidget)
        self.frame_XZ.setFrameShape(QFrame.NoFrame)
        self.frame_XZ.setObjectName("frame_XZ")
        self.frame_XZ.setStyleSheet("background-color: #808080; border: 2px solid #666;")
        self.id_XZ = "XZ"
        ...
        ren_xz.SetBackground(0.5, 0.5, 0.5)  # 灰色背景 #808080
        self.vtkWidget3.GetRenderWindow().AddRenderer(ren_xz)
        self.viewer_XZ = None  # 初始不设置viewer

        self.horizontalLayout_3.addWidget(self.vtkWidget3)

        self.verticalSlider_XZ = QScrollBar(self.centralwidget)
        self.verticalSlider_XZ.setObjectName(u"verticalSlider_XZ")
        self.verticalSlider_XZ.setStyleSheet(self.verticalSlider_XY.styleSheet())
        self.verticalSlider_XZ.setOrientation(Qt.Vertical)
        self.verticalSlider_XZ.setEnabled(False)  # 初始禁用滑块
        self.horizontalLayout_3.addWidget(self.verticalSlider_XZ)
        ...
        self.frame_Volume.setStyleSheet("background-color: #808080; border: 2px solid #666;")
        ...
        self.reader_stl_renderer.SetBackground(0.5, 0.5, 0.5)  # 灰色背景 #808080
        ...
        self.horizontalLayout_4.addWidget(self.vtkWidget4)

        # Volume滑块 - 用于控制体绘制的透明度或切片
        self.verticalSlider_Volume = QScrollBar(self.centralwidget)
        self.verticalSlider_Volume.setObjectName(u"verticalSlider_Volume")
        ...
        self.verticalSlider_Volume.setEnabled(False)  # 初始禁用滑块
        self.horizontalLayout_4.addWidget(self.verticalSlider_Volume)
        self.gridLayout_2.addLayout(self.horizontalLayout_4, 1, 1, 1, 1)

        # 创建切片标签（用于代码逻辑，但不显示在底部）
    ...
    def _create_menubar(self, QMainWindow):
        ...
        self.segmentationMenu = QMenu("SAM-Med2D Seg", self.menubar)
    ...
    def retranslateUi(self, QMainWindow):
        ...
        QMainWindow.setWindowTitle(_translate("SAMMed", "SAM-Med-Viewer"))
        ...
        self.segmentationMenu.setTitle(_translate("SAMMed", "&SAM-Med2D Seg"))
    ...
    def _apply_scaling(self):
        ...
        # 计算缩放后的字体大小 - 微软雅黑字体
        title_font_size = max(14, int(16 * scale_factor))
    ...
    def on_actionAdd_Load_Universal_model(self):
        ...
        # -----------------------------------------------------
        # 设置通用模型的权重路径
        self.args.sam_checkpoint = "./sam-med2d_b.pth"
